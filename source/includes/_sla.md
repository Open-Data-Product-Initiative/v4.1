...
| **$ref** | filepath or valid URL | - | Define the SLA in external file for reuse purposes, example  $ref: 'https://example.org/slas/all-packages.yaml' See example. <br/><br/>The same pattern can be used in individual SLA profiles instead of doing it inline. See example. |
